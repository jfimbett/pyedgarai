...
# Lightweight version to avoid importing heavy modules at import time
# Mirrors behavior of pyedgarai.clean_account_name for examples
...
def clean_account_name(name: str) -> str:
    ...
    # Title-case words already in upper remain
    parts = [w.capitalize() if w and w[0].islower() else w for w in s.split()]
    s = ''.join(parts)
    # Specific normalization used in library
    if s == "Longterm Debt Excluding Current Maturities":
        s = "Long Term Debt Noncurrent"
    return s
...
class CIKTickersResponse(BaseModel):
    cik_tickers: Dict[str, List[str]] = Field(description="Dictionary where keys are CIKs and values are corresponding tickers", example={
            "0000001750": ["AIR"],
            "0000001800": ["ABT"],
            "0000001961": ["WDDD"]
        })
...
class CIKNamesResponse(BaseModel):
    cik_names: Dict[str, List[str]] = Field(description="Dictionary where keys are CIKs and values are company names", example={
            "0000001750": ["AAR CORP"],
...
class AccountResponse(BaseModel):
    ...
    data: List[Dict] = Field(description="List of data points for different companies", example=[{
...
class CompanyResponse(BaseModel):
    ...
    taxonomy: str = Field(description="Account taxonomy", example="us-gaap")
    units: Dict[str, List[Dict[str, Any]]] = Field(description="Unit values", example={"USD": [{"accn": "0001193125-10-012091", "val": 1511000000}]})
...
class ValuationMetricsResponse(BaseModel):
    avg_multiple: Dict[str, float] = Field(description="Average valuation multiples (Price to Earnings, Price to Book, Enterprise to EBITDA)", example={"priceToEarnings": 15.5, "priceToBook": 2.3, "enterpriseToEbitda": 10.1})
    variables: List[Dict] = Field(description="List of valuation metrics for each ticker", example=[{
                                      "ticker": "AAPL",
                                      "eps": 6.35,
...
class ComparablesPrivateRequest(BaseModel):
    api_token: str = Field(description="API token for authentication", example="t3stt@ken")
    name: str = Field(description="Name of the private company", example="TechCorp Inc.")
    sic_code: str = Field(description="Two-digit SIC code of the company", example="73")
    profitability: float = Field(description="Profitability measured as net income over assets (ROA)", example=0.15)
    growth_rate: float = Field(description="Growth rate of assets in the last 5 years (CAGR)", example=0.12)
    capital_structure: float = Field(description="Capital structure measured as debt over equity", example=0.35)

class ComparablesPrivateResponse(BaseModel):
    target_company: Dict[str, Any] = Field(description="Details of the target private company", example={
        "name": "TechCorp Inc.",
        "sic_code": "73",
        "profitability": 0.15,
        "growth_rate": 0.12,
        "capital_structure": 0.35
    })
    comparables: List[Dict[str, Any]] = Field(description="List of comparable public companies with financial metrics", example=[{
        "cik": 320193,
        "name": "Apple Inc",
        "ticker": "AAPL", 
        "sic": "3571",
        "profitability": 0.14,
        "growth_rate": 0.11,
        "capital_structure": 0.28,
        "market_cap": 2400000000000,
        "price_to_earnings": 23.6,
        "price_to_book": 40.5,
        "enterprise_to_ebitda": 20.5,
        "distance": 0.023
    }])
    total_found: int = Field(description="Total number of comparable companies found", example=5)
    method: str = Field(description="Method used for analysis", example="private_comparables")
    sic_sector: str = Field(description="SIC sector used for filtering", example="73")
